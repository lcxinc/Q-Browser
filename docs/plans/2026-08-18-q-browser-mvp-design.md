...
The host starts one worker for the active package. The Windows sandbox adapter creates an AppContainer identity for the package, supplies a read-only package location and writable worker temp area, removes ambient network access, and attaches Job Object limits for memory, process count, and termination-on-host-exit.
