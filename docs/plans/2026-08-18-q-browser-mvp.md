...
Construct launches only through a move-only `SandboxTrustBoundary`. It rejects volume/broad/overlapping/reparse or Worker-writable approved roots, holds stable handles plus file identity, and rechecks owner/DACL safety through those handles before launch. It permits only strict package/temp descendants and requires the executable to match the canonical path and file identity of a captured immutable-runtime file. Package grants are read-only without execute, temp grants are read/write without execute, and only each pre-recorded runtime object receives non-inherited read/execute; additions after boundary construction receive no Worker ACE or launch authority. Validate Job limits before the first grant. Stage only the Qt-linked helper and Qt6Core DLL in the runtime proof; under `BUILD_TESTING` reproduce zero and `lpacCom` loader denial plus exact `registryRead` success, independently prove both token group queries succeeded before asserting AAP absence, and exclude matrix hooks from production. Preserve exact HRESULT/DWORD failures in typed results rather than consulting ambient `GetLastError()` after helper calls. Normal owners must call checked ACL/process close APIs and surface rollback failure; destructors are best-effort cleanup only.
